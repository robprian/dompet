...
void main() {
  ...
  group('NotificationParserRegistry', () {
    test('returns the highest confidence candidate', () {
      ...
      final candidate = registry.parse(payload(body: 'Pembayaran QRIS Rp25.000 di KOPI ABC berhasil'));
      expect(candidate, isNotNull);
      expect(candidate!.method, PaymentMethod.qris);
      expect(candidate.confidence, greaterThanOrEqualTo(0.9));
    });
    ...
    test('returns null for unrecognized notifications', () {
      const registry = NotificationParserRegistry();
      expect(registry.parse(payload(body: 'Cuaca hari ini cerah')), isNull);
    });

    test('fingerprints differ across packages', () {
      const registry = NotificationParserRegistry();
      final a = registry.parse(payload(body: 'Pembayaran QRIS Rp25.000 di KOPI ABC berhasil', package: 'a.bank'));
      final b = registry.parse(payload(body: 'Pembayaran QRIS Rp25.000 di KOPI ABC berhasil', package: 'b.bank'));
  ...
  group('TransactionCandidate', () {
    test('fingerprints are stable for identical inputs', () {
      ...
      final a = TransactionCandidate.makeFingerprint(
      ...
      final b = TransactionCandidate.makeFingerprint(
        sourcePackage: 'com.bank',
        amount: 25000,
        type: DetectionTransactionType.expense,
        merchant: 'KOPI ABC',
    ...
    test('canAutoImport follows the 0.90 tier rule', () {
      ...
      expect(candidate.canAutoImport, isTrue);
    });
  });
}
